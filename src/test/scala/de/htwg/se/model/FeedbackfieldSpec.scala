    ...
    "empty" should {
      ...
      "be initially filled with Feedback.Nothing" in {
        ...
        for (i <- 0 until 4; j <- 0 until 4) {
          emptyField.getFeedbackMatrix.cell(i, j) should be(Feedback.Nothing)
        }
      }
    }
    "filled" should {
      ...
      "calculate the correct feedback" in {
        de.htwg.se.model.FeedbackField.calculateFeedback(field, 0, 0) should be(Feedback.PositionCorrect)
      }
    }
  }
} 
